...
@WebServlet("/home/candidatovaga/delete")
public class CandidatarVagaDelServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try(DAOFactory daoFactory = new DAOFactory()) {
            ...
            if(dcv.findById(user.getId(), idVaga).isPresent() && dV.findById(idVaga).isPresent()){

                dV.findById(idVaga).ifPresent(vg -> {

                dcv.delete(user.getId(), vg.getId());
